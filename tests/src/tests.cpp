...
TEST_CASE("linked_list")
{
  ...
  ns = free_list(ns);
	...
	ns = make_node (1,NULL); // changed to NULL
	sum=sum_squares(ns);
	REQUIRE(sum==1);
	...
	//ns is 1->2->3
  	mns = map (ns, square);
	//ns is 1->4->9
	sum=sum_squares(mns); //added 'm' with permission from Prof. Bennedsen
	//1+16+81 = 98
	REQUIRE(sum==98);
	ns = free_list(mns);
}
...
TEST_CASE("btree")
{
  ...
  REQUIRE (Full (root) == 0); // changed this test with permission from Prof. Bennedsen since a binary search tree of linked list can never be full.
